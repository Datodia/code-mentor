...
export default function MarkdownPreview({ content, className = '', size = 'lg', enableImageUpload = false }: Props) {
  ...
  const customComponents: Components = {
    h1: ({ ...props }) => <h1 className={`${isLarge ? 'text-4xl mb-6 mt-8' : 'text-3xl mb-4 mt-6'} font-bold text-primary`} {...props} />,
    h2: ({ ...props }) => <h2 className={`${isLarge ? 'text-3xl mt-8 mb-4' : 'text-2xl mt-6 mb-3'} font-semibold text-primary`} {...props} />,
    h3: ({ ...props }) => <h3 className={`${isLarge ? 'text-2xl mt-6 mb-3' : 'text-xl mt-5 mb-2'} font-semibold text-primary/80`} {...props} />,
    h4: ({ ...props }) => <h4 className={`${isLarge ? 'text-xl mt-5 mb-2' : 'text-lg mt-4 mb-2'} font-semibold text-primary/70`} {...props} />,
    h5: ({ ...props }) => <h5 className={`${isLarge ? 'text-lg mt-4 mb-2' : 'text-base mt-3 mb-2'} font-semibold text-primary/60`} {...props} />,
    h6: ({ ...props }) => <h6 className={`${isLarge ? 'text-base mt-4 mb-2' : 'text-sm mt-3 mb-2'} font-semibold text-primary/50`} {...props} />,

    p: ({ ...props }) => <p className={`${isLarge ? 'text-lg' : 'text-base'} text-foreground mb-4`} {...props} />,
    ...
    hr: () => <hr className={`${isLarge ? 'my-8' : 'my-6'} border-t border-border`} />,

    ul: ({ ...props }) => <ul className={`list-disc list-outside pl-6 ${isLarge ? 'mb-6' : 'mb-4'} text-foreground space-y-1`} {...props} />,
    ol: ({ ...props }) => <ol className={`list-decimal list-outside pl-6 ${isLarge ? 'mb-6' : 'mb-4'} text-foreground space-y-1`} {...props} />,
    li: ({ ...props }) => <li className="mb-1" {...props} />,

    blockquote: ({ ...props }) => (
      <blockquote
        className={`pl-4 italic border-l-4 border-primary/30 text-foreground/80 ${isLarge ? 'my-6' : 'my-4'} py-1`}
        {...props}
      />
    ),
    ...
    th: ({ ...props }) => <th className={`px-4 ${isLarge ? 'py-3' : 'py-2'} text-left font-semibold text-foreground border border-border`} {...props} />,
    td: ({ ...props }) => <td className={`px-4 ${isLarge ? 'py-3' : 'py-2'} text-foreground border border-border`} {...props} />,
    ...
    img: ({ src, alt, ...props }) => {
      if (!src || (typeof src === 'string' && src.startsWith('http')) || !enableImageUpload) {
        return (
          <img
            src={src || ''}
            alt={alt || ''}
            className={`rounded-lg ${isLarge ? 'my-6' : 'my-4'} max-w-full h-auto`}
            {...props}
          />
        )
      }
      ...
      return (
        <div className={`relative ${isLarge ? 'my-6 h-[300px] md:h-[400px]' : 'my-4 h-[200px]'} w-full`}>
          <Image
            src={src as string}
            alt={alt || ''}
            fill
