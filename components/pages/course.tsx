      ...
      <div className="md:flex gap-8">
        ...
        <div className="w-full flex flex-col">
          <div className="mb-2 md:mb-6">
            <h1 className="text-center text-base md:text-lg lg:text-xl ">
              {course.title}
            </h1>
          ...
          <div className="flex flex-col gap-2 md:gap-4">
            <h3 className="text-sm md:text-base lg:text-lg flex items-center gap-1">
              <Banknote className="w-4 md:w-6" />
              კურსის ღირებულება:{" "}
              <span className="font-medium">{course.price}₾</span>
            </h3>
            <h2 className="text-sm md:text-base lg:text-lg flex items-center gap-1">
              <Hourglass className="w-4 md:w-6" /> ლექციების რაოდენობა:{" "}
              <span className="font-medium">{course.totalLessons}</span>
            </h2>
            <h2 className="text-sm md:text-base lg:text-lg flex items-center gap-1">
              <GraduationCap className="w-4 md:w-6" /> დონე:{" "}
              <span className="font-medium">{course.level}</span>
            </h2>
            <h2 className="text-sm md:text-base lg:text-lg flex items-center gap-1">
              <Users className={"w-4 md:w-6"} /> სუდენტების რაოდენობა:{" "}
              <span className="font-medium">{course.totalEnrollments}</span>
            </h2>
            <h2 className="text-sm md:text-base lg:text-lg flex items-center gap-1">
              <Laptop className={"w-4 md:w-6"} /> ფორმატი:{" "}
              <span className="font-medium">ონლაინ</span>
            </h2>
            <>
              {loading ? (
                <Button disabled className="md:text-base md:w-[240px]">
                  <Loader2 className="animate-spin" />
                </Button>
              ) : (
                <Button
                  onClick={() => handleBuyCourse(course)}
                  className="md:text-base md:w-[240px]"
                >
                  ყიდვა
                </Button>
              )}
